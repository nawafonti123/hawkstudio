...
# ============================================================================
# قاعدة البيانات - الإصدار المُصلَح النهائي
# ============================================================================
...
class Database:
    ...
    def _init_pool(self):
        ...
        try:
            pool_config = {
                ...
                'use_unicode': True,
                'autocommit': True,
                'use_pure': True,  # 🔴 ضروري لتجنب مشاكل C
                'connection_timeout': 5,
                'auth_plugin': 'mysql_native_password'  # 🔴 مهم لـ MySQL 8+
            }
            
            self.pool = mysql.connector.pooling.MySQLConnectionPool(**pool_config)
            print("✅ تم تهيئة Connection Pool")
            
        except Exception as e:
            print(f"❌ فشل في تهيئة Connection Pool: {e}")
            self.pool = None
    ...
    def get_connection(self):
        ...
        try:
            ...
            print(f"❌ خطأ في الحصول على اتصال: {e}")
            return None
    ...
    def execute_select(self, query, params=None):
        """تنفيذ استعلام SELECT - اتصال جديد لكل استعلام"""
        conn = None
        cursor = None
        try:
            conn = self.get_connection()
            ...
            print(f"❌ خطأ في SELECT: {e}")
            print(f"   الاستعلام: {query}")
            print(f"   المعاملات: {params}")
            
            # 🔴 التحقق من نوع الخطأ المحدد
            if "bytearray index out of range" in str(e):
                print("⚠️  خطأ bytearray - إعادة تهيئة الاتصال")
                self._init_pool()  # إعادة تهيئة الـ Pool
            
            return None
            
        except Exception as e:
            print(f"❌ خطأ غير متوقع في SELECT: {e}")
            return None
            
        finally:
            # 🔴 هذا هو المفتاح: إغلاق الموارد دائمًا
            if cursor:
                cursor.close()
            if conn:
                conn.close()
    ...
    def execute_write(self, query, params=None):
        ...
        try:
            ...
            print(f"❌ خطأ غير متوقع في Write: {e}")
            if conn:
                conn.rollback()
            return None
    ...
    def fix_database_issues(self):
        ...
        print("🔧 فحص وإصلاح مشاكل قاعدة البيانات...")
        
        try:
            # 1. التحقق من الجداول
            print("1. التحقق من الجداول...")
            ...
            """
            tables = self.execute_select(query, (self.database,))
            
            if not tables:
                print("❌ جدول admin_users غير موجود!")
                # إنشاء الجدول مباشرة
                query = """
                CREATE TABLE IF NOT EXISTS admin_users (
                    id INT AUTO_INCREMENT PRIMARY KEY,
            ...
            # 2. التحقق من مستخدم admin
            print("2. التحقق من مستخدم admin...")
            ...
            if not admin_user:
                ...
                if result:
                    print(f"✅ تم إضافة مستخدم admin")
                else:
                    print("⚠️  فشل في إضافة مستخدم admin")
            else:
                print(f"✅ مستخدم admin موجود")
                
                # التحقق من أن password_hash صالح
                stored_hash = admin_user[0].get('password_hash', '')
                if not stored_hash or stored_hash == '':
                    print("⚠️  password_hash فارغ، جاري إصلاحه...")
                    password = "admin123"
                    password_hash = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt())
                    
                    query = "UPDATE admin_users SET password_hash = %s WHERE username = 'admin'"
                    result = self.execute_write(query, (password_hash.decode('utf-8'),))
                    if result:
                        print(f"✅ تم إصلاح password_hash")
                elif not stored_hash.startswith('$2'):
                    print("⚠️  password_hash ليس بصيغة bcrypt، جاري إصلاحه...")
                    password = "admin123"
                    password_hash = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt())
                    ...
                    result = self.execute_write(query, (password_hash.decode('utf-8'),))
                    if result:
                        print(f"✅ تم تحويل password_hash إلى bcrypt")
            
            # 3. التحقق من إعدادات الموقع الأساسية
            print("3. التحقق من إعدادات الموقع...")
            basic_settings = [
            ...
            for key, value in basic_settings:
                ...
                if not setting_exists:
                    print(f"⚠️  إعداد {key} غير موجود، جاري إضافته...")
                    ...
                    if result:
                        ...
                        print(f"⚠️  فشل في إضافة إعداد {key}")
    ...
    def init_admin_user(self):
        """تهيئة مستخدم المسؤول فقط"""
        try:
            # التحقق أولاً من وجود مستخدم admin
            query = "SELECT id FROM admin_users WHERE username = 'admin'"
            existing_user = self.execute_select(query)
            ...
            print(f"✅ تم إضافة مستخدم admin فقط")
            return True
            
        except Exception as e:
            print(f"❌ خطأ في تهيئة مستخدم المسؤول: {e}")
            return False
    ...
    def setup_database(self):
        ...
        try:
            ...
            if not self.create_tables():
                print("⚠️  حدثت مشاكل في إنشاء الجداول")
            
            # 3. إصلاح أي مشاكل
            print("🔧 إصلاح مشاكل قاعدة البيانات...")
            if not self.fix_database_issues():
                print("⚠️  حدثت مشاكل في إصلاح قاعدة البيانات")
            
            # 4. تهيئة مستخدم المسؤول فقط
            print("👤 تهيئة مستخدم المسؤول...")
            if not self.init_admin_user():
                print("⚠️  حدثت مشاكل في تهيئة مستخدم المسؤول")
            
            print("\n🎉 تم إعداد قاعدة البيانات بنجاح!")
            print("   ✅ تم الاتصال بقاعدة البيانات")
            print("   ✅ تم إنشاء/تحديث الجداول")
            print("   ✅ تم إضافة مستخدم admin فقط")
            print("   ❌ لا توجد بيانات افتراضية (مشاريع/طلبات)")
            print("   📝 يمكنك إضافة البيانات يدوياً من لوحة التحكم")
            
            return True
            
        except Exception as e:
            print(f"❌ خطأ غير متوقع في إعداد قاعدة البيانات: {e}")
            return False
...
def maintenance_page():
    ...
    # إذا لم يكن وضع الصيانة مفعلاً، توجيه إلى الصفحة الرئيسية
    return redirect('/')
...
def get_projects():
    ...
    try:
        ...
        return create_response([], f'حدث خطأ: {str(e)}', 500, False)
...
def get_site_status():
    ...
    try:
        ...
        all_settings = db.execute_select(query)
        ...
        return create_response({
            ...
            'site_title': settings_dict.get('site_title', 'HawkStudio'),
            'site_description': settings_dict.get('site_description', 'هندسة الويب بمنهجية البرمجيات أولاً')
...
def create_project_request():
    ...
    try:
        ...
        if result is None:
            return create_response(None, 'فشل في حفظ الطلب', 500, False)
        
        # Get the created request
        query = "SELECT * FROM project_requests WHERE id = LAST_INSERT_ID()"
        new_request = db.execute_select(query)
        
        if new_request:
            new_request = new_request[0]
            # Convert datetime to string
            for date_field in ['created_at', 'updated_at']:
                if new_request.get(date_field) and hasattr(new_request[date_field], 'isoformat'):
                    new_request[date_field] = new_request[date_field].isoformat()
        
        return create_response(new_request, 'تم استلام طلبك بنجاح', 201)
        
    except Exception as e:
        app.logger.error(f"Error in create_project_request: {str(e)}")
        return create_response(None, f'حدث خطأ: {str(e)}', 500, False)
...
def admin_login():
    ...
    try:
        ...
        if not stored_hash:
            ...
            print(f"✅ تم إنشاء hash جديد للمستخدم {username}")
        elif not stored_hash.startswith('$2'):
            # تحويل hash قديم إلى bcrypt
            password_hash = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt())
            db.execute_write(
                "UPDATE admin_users SET password_hash = %s WHERE id = %s",
                (password_hash.decode('utf-8'), user['id'])
            )
            print(f"🔄 تم تحويل hash لـ {username} إلى bcrypt")
        else:
            ...
            try:
                ...
                # إنشاء hash جديد في حالة الخطأ
                password_hash = bcrypt.hashpw(password.encode('utf-8'), bcrypt.gensalt())
                db.execute_write(
                    "UPDATE admin_users SET password_hash = %s WHERE id = %s",
                    (password_hash.decode('utf-8'), user['id'])
                )
                print(f"✅ تم إصلاح hash كلمة المرور للمستخدم {username}")
        ...
        return create_response(None, f'حدث خطأ في تسجيل الدخول: {str(e)}', 500, False)
...
def admin_create_project():
    ...
    try:
        ...
        query = "SELECT * FROM projects WHERE id = LAST_INSERT_ID()"
        new_project = db.execute_select(query)
        
        if new_project:
            new_project = new_project[0]
            # Convert date objects to string
            for date_field in ['project_date', 'created_at', 'updated_at']:
                if new_project.get(date_field) and hasattr(new_project[date_field], 'isoformat'):
                    new_project[date_field] = new_project[date_field].isoformat()
        
        return create_response(new_project, 'تم إضافة المشروع بنجاح', 201)
...
def admin_update_project(project_id):
    ...
    try:
        ...
        updated_project = db.execute_select(query, (project_id,))
        
        if updated_project:
            updated_project = updated_project[0]
            # Convert date objects to string
            for date_field in ['project_date', 'created_at', 'updated_at']:
                if updated_project.get(date_field) and hasattr(updated_project[date_field], 'isoformat'):
                    updated_project[date_field] = updated_project[date_field].isoformat()
        
        return create_response(updated_project, 'تم تحديث المشروع بنجاح')
...
def admin_get_stats():
    ...
    try:
        ...
        result = db.execute_select(query)
        stats['total_projects'] = result[0]['count'] if result and result[0] else 0
        
        # Active projects
        query = "SELECT COUNT(*) as count FROM projects WHERE is_active = TRUE"
        result = db.execute_select(query)
        stats['active_projects'] = result[0]['count'] if result and result[0] else 0
        
        # Total requests
        query = "SELECT COUNT(*) as count FROM project_requests"
        result = db.execute_select(query)
        stats['total_requests'] = result[0]['count'] if result and result[0] else 0
        
        # New requests
        query = "SELECT COUNT(*) as count FROM project_requests WHERE status = 'new'"
        result = db.execute_select(query)
        stats['new_requests'] = result[0]['count'] if result and result[0] else 0
        
        # Recent projects (last 5)
        query = "SELECT * FROM projects ORDER BY created_at DESC LIMIT 5"
        recent_projects = db.execute_select(query)
        if recent_projects:
            for project in recent_projects:
                if project.get('created_at') and hasattr(project['created_at'], 'isoformat'):
                    project['created_at'] = project['created_at'].isoformat()
        stats['recent_projects'] = recent_projects or []
        
        # Recent requests (last 5)
        query = "SELECT * FROM project_requests ORDER BY created_at DESC LIMIT 5"
        recent_requests = db.execute_select(query)
        if recent_requests:
            for req in recent_requests:
                if req.get('created_at') and hasattr(req['created_at'], 'isoformat'):
                    req['created_at'] = req['created_at'].isoformat()
        stats['recent_requests'] = recent_requests or []
...
if __name__ == '__main__':
    ...
    print("   وضع الصيانة: http://localhost:5000/maintenance")
    ...
    print("   📝 يمكنك إضافة البيانات يدوياً من لوحة التحكم")
    print("\n⚙️  خاصية وضع الصيانة:")
    print("   - يمكن تفعيل/تعطيل وضع الصيانة من لوحة التحكم")
    print("   - عند التفعيل: المستخدمون ينتقلون إلى صفحة الصيانة")
    print("   - عند التعطيل: الموقع يعمل بشكل طبيعي")
    print("\n🔧 إصلاح المشاكل السابقة:")
    print("   ✅ Connection Pool (حل مشكلة الاتصال)")
    print("   ✅ use_pure=True (حل مشكلة C extensions)")
    print("   ✅ auth_plugin (حل مشكلة MySQL 8+)")
    print("   ✅ إغلاق الموارد دائمًا (حل مشكلة الذاكرة)")
    print("\n⏹️  اضغط Ctrl+C لإيقاف السيرفر")
    
    # إضافة خيارات إضافية لتجنب مشاكل الاتصال
    app.run(
        debug=True, 
        port=5000, 
        host='0.0.0.0',
